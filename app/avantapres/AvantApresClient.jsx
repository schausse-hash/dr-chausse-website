...
import { Star, ExternalLink, Phone, ChevronLeft, ChevronRight } from 'lucide-react'
import Link from 'next/link'

function CasCard({ cas }) {
  const [showApres, setShowApres] = useState(false)
  const hasPhotos = cas.photo_avant_url || cas.photo_apres_url

  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
      {/* Zone photo avec toggle avant/après */}
        ...
        {hasPhotos ? (
          <>
            ...
            <div className="relative h-72 overflow-hidden">
              <img
                src={showApres ? (cas.photo_apres_url || cas.photo_avant_url) : (cas.photo_avant_url || cas.photo_apres_url)}
                alt={showApres ? `Après — ${cas.titre}` : `Avant — ${cas.titre}`}
            ...
            {/* Boutons toggle — uniquement si les deux photos existent */}
            {cas.photo_avant_url && cas.photo_apres_url && (
...
export default function AvantApresClient({ cas }) {
