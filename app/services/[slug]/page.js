...
export async function generateMetadata({ params }) {
  ...
  const { data } = await supabase.from('services').select('title, excerpt, seo_title, seo_description').eq('slug', params.slug).eq('locale', 'fr').single()
  if (!data) return {}
  return {
    ...
    <main>
      <div className="relative pt-20 bg-dental-900">
        <div className="max-w-4xl mx-auto px-6 py-20 text-white text-center">
          ...
          <div className="text-dental-300 text-sm font-medium uppercase tracking-widest mb-3">{service.category}</div>
          <h1 className="font-display text-4xl md:text-5xl mb-4">{service.title}</h1>
          {service.excerpt && <p className="text-xl text-white/80 max-w-2xl mx-auto">{service.excerpt}</p>}
        </div>
      </div>
          ...
          {content.length > 0 ? (
            <div className="space-y-10">
              {content.map((section, i) => (
                <div key={i}>
                  {section.titre && <h2 className="font-display text-2xl text-charcoal mb-3">{section.titre}</h2>}
                  {section.texte && <p className="text-warm-gray leading-relaxed">{section.texte}</p>}
                  {section.liste && (
